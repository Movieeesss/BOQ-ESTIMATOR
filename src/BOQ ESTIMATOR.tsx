...
const ENG_LOGIC = { cement: 0.42, steel: 4.0, sand: 1.8, bricks: 22, paint: 0.25, tiles: 1.1 };
...
const IMAGE_MATERIAL_LIST = [
  ...
  { item: "Electrical", spec: "Finolex Pipes & GM Modular Switches", unit: "LS", constant: 0 }
];
...
interface BOQItem { id: number; desc: string; qty: number; rate: number; unit: string; type: 'construction' | 'expense'; }

// --- 2. FAST INPUT COMPONENT (ZERO LAG) ---
...
export default function MasterProfessionalBOQ() {
  ...
  // COMBINED ITEMS FROM IMAGES 1 & 2
  const [items, setItems] = useState<BOQItem[]>([
    { id: 1, desc: "Ground Floor Construction Area", qty: 1057.5, rate: 2500, unit: "Sft", type: 'construction' },
    { id: 2, desc: "First Floor Construction Area", qty: 1057.5, rate: 2500, unit: "Sft", type: 'construction' },
    { id: 3, desc: "Brick Water sump (2000 L) + 1 HP motor", qty: 1, rate: 150000, unit: "Nos", type: 'construction' },
    { id: 4, desc: "Borewell with 1 HP Jet Pump", qty: 1, rate: 80000, unit: "Nos", type: 'expense' },
    { id: 5, desc: "Plan Approval Fee and Expenses", qty: 1, rate: 75000, unit: "LS", type: 'expense' },
    { id: 6, desc: "3 Phase E.B. Supply & Temp EB", qty: 2, rate: 50000, unit: "Nos", type: 'expense' },
    { id: 7, desc: "Property tax assessment", qty: 1, rate: 50000, unit: "LS", type: 'expense' },
    { id: 8, desc: "UGD connection", qty: 1, rate: 40000, unit: "LS", type: 'expense' }
  ]);

  const totals = useMemo(() => {
    const constructionTotal = items.filter(i => i.type === 'construction').reduce((sum, i) => sum + (i.qty * i.rate), 0);
    const expenseTotal = items.filter(i => i.type === 'expense').reduce((sum, i) => sum + (i.qty * i.rate), 0);
  ...
  const generatePDF = () => {
    ...
    // TABLE 1: MAIN CONSTRUCTION
    autoTable(doc, {
      startY: 30,
      head: [['Construction Work Description', 'Quantity', 'Rate', 'Amount']],
      body: items.filter(i => i.type === 'construction').map(i => [i.desc, `${i.qty} ${i.unit}`, `Rs.${i.rate}`, `Rs.${(i.qty * i.rate).toLocaleString()}`]),
    ...
    autoTable(doc, {
      ...
      body: items.filter(i => i.type === 'expense').map(i => [i.desc, `${i.qty} ${i.unit}`, "Approx", `Rs.${(i.qty * i.rate).toLocaleString()}`]),
      foot: [['', '', 'ADDITIONAL EXPENSES TOTAL', `Rs. ${totals.expenseTotal.toLocaleString()}`]],
      theme: 'grid', headStyles: { fillColor: [80, 80, 80] }
    });
    ...
    doc.text(`GRAND TOTAL ESTIMATED VALUE: Rs. ${totals.grandTotal.toLocaleString()}`, 14, finalY);

    // TABLE 3: MATERIAL LOGIC
    const matRows = IMAGE_MATERIAL_LIST.map(m => [m.item, m.spec, m.constant > 0 ? `${Math.round(builtArea * m.constant)} ${m.unit}` : "Standard"]);
    autoTable(doc, {
      startY: finalY + 5,
      head: [[{ content: 'MATERIAL CONSUMPTION (Based on Reference Area)', colSpan: 3, styles: { halign: 'center' } }]],
      body: matRows,
    ...
    QUOTATION_NOTES.forEach((note, index) => {
      doc.text(`- ${note}`, 14, noteY + 5 + (index * 5));
    });
  ...
  return (
    <div style={containerStyle}>
      <div style={headerStyle}>
        ...
        <button onClick={() => setItems([...items, { id: Date.now(), desc: "New Work", qty: 1, rate: 0, unit: "Nos", type: 'construction' }])} style={addBtn}>+ ADD ITEM</button>
      </div>
...
const areaInput = { width: '100%', padding: '8px', border: '2px solid #0369a1', borderRadius: '8px', fontSize: '18px', fontWeight: 'bold' as const, textAlign: 'center' as const };
